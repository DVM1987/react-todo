pipeline {
  ...
  environment {
    dockerimagename = "vietnambkk24/react-todo"
    dockerImage = ""
  ...
  stages {
    ...
    stage('Build image') {
      steps{
        container('docker') {
          script {
            sh 'docker build --network=host -t vietnambkk24/react-todo .'
          }
        }
      }
    }
      ...
      steps{
        container('docker') {
          script {
            ...
            sh 'docker tag vietnambkk24/react-todo vietnambkk24/react-todo'
            sh 'docker push vietnambkk24/react-todo:latest'
          }
        }
      }
    }
